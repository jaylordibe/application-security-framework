...
**Deriving the oracle from the application's own metadata, and accounting honestly for
what was not tested, is the product.**
...
1. **Oracle derivation.** Turn the application's own authorization metadata — via
   out-of-process, language-native adapters — into a normalized, machine-checkable
   expectation of who may do what to which resource. Nobody does this. Hadrian requires
   you to hand-write `roles.yaml`; an LLM guessing it is a hypothesis, not truth.
2. **Honest coverage accounting.** Route, operation, identity, resource, tenant and
   workflow coverage, with every blocked test itemised and attributed to a cause.
   **Verified absent from every open-source tool examined**, Hadrian included.
3. **Provenance and evidence semantics.** Every fact carries where it came from
...
4. **Tenancy as a real dimension**, and **verification below the HTTP layer** where the
   evidence source is actually available and trustworthy.
